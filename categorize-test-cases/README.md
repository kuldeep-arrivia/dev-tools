...
###  This application categorizes the test cases (exported from Azure DevOps portal) into categories CRITICAL, High, Medium and Low - using a local LLM.
...
## Prerequisites
 ...
 #### Download the default model 'qwen2.5:7b' using Ollama command (this model was the smallest model giving good results - you can go for larger model based on your hardware configuration)
 ```bash
...
```bash
 http://localhost:8501/ 
```
#### where you can select the folder name where you had exported excel sheet (not the root folder, but exact sub-folder having excel sheets) - 
...
```
#### b. Launch the below URL launched, 
```bash
 http://localhost:8501/ 
```
